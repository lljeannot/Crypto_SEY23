...
data_Enneabel <- data %>% 
  ...
  mutate(nsite = n()) %>% 
  ungroup() %>% 
  mutate(meannsite = mean(nsite, na.rm = T),
         sdsite = sd(nsite, na.rm = T)) %>% 
  distinct(GENSPE, ntot, meannsite, sdsite)

data_Enneabel

data_Eviobipu <- data %>% 
  filter(GENSPE == "Eviobipu") %>% 
  mutate(ntot = n()) %>% 
  group_by(Site) %>% 
  mutate(nsite = n()) %>% 
  ungroup() %>% 
...
ocean_cropped_F <- st_crop(ocean,xmin = 55.93, xmax = 55.97, ymin = -4.578, ymax = -4.588)
...
ocean_F <- st_difference(ocean_cropped_F, out_F)
...
noddy <- noddy[st_geometry_type(noddy) == "POINT", ]
cropped_noddy <- st_crop(noddy, xmin = 55.93, xmax = 55.97, ymin = -4.575, ymax = -4.588)

# Define grid for interpolation
grid <- st_make_grid(out_F, n = c(60, 60))  
grid_sf <- st_sf(geometry = grid)
grid_sf$Description <- 0 

grid_with_points <- st_join(grid_sf, cropped_noddy, left = TRUE)

grid_with_points$Description <- ifelse(
  !is.na(grid_with_points$Description.y),  
  grid_with_points$Description.y,          
  grid_with_points$Description.x           
)

grid_with_points <- grid_with_points %>%
  select(geometry, Description)

# Inverse distance weighted interpolation
g <- gstat::gstat(formula = Description ~ 1, locations = grid_with_points, set = list(idp = 1)) 
idw_result <- predict(g, grid_with_points) #takes a long time

interp_df <- as.data.frame(idw_result, xy = TRUE) %>%
  mutate(var1.pred = as.numeric(var1.pred)) %>%
...
map_F <- ggplot() +
  ...
  geom_contour_fill(aes(x = x, y = y, z = var1.pred, fill = after_stat(level_mid)), bins = 10, data = interp_df) +
  geom_sf(data = contours_clipped, aes(geometry = geometry), alpha = 0.15) +  
  geom_sf(data = ocean_F, fill = "white", color = "NA") +
  geom_sf(data = out_F, fill = "transparent") +
  geom_point(data = site_data, aes(y = Lat, x = Lon, col = Depth, shape = Community), size = 2) +
  annotate(geom = "text", x = 55.948, y = -4.5806, label = "La Cour Beach", fontface = "italic", color = "black", size = 2.7) +
  ...
  annotation_scale(data = out_F, location = "bl", width_hint = 0.05, height = unit(0.2, "cm"), text_cex = 0.5, text_pad = unit(0.15, "cm")) +
...
grid_noddy <- as(extent(55.93, 55.97, -4.588, -4.575), "SpatialPolygons")

grid <- st_make_grid(out_F, n = c(8, 8))  
grid_sf <- st_sf(geometry = grid)
grid_sf$Description <- 0 

grid_with_points <- st_join(grid_sf, cropped_tern, left = TRUE)
...
# Inverse distance weighted interpolation
g <- gstat::gstat(formula = Description ~ 1, locations = grid_with_points, set = list(idp = 2)) 
idw_result_tern <- predict(g, grid_with_points) #takes several minutes

#coords_idw_result_tern <- st_coordinates(idw_result_tern)

interp_df <- as.data.frame(idw_result_tern, xy = TRUE) %>%
  mutate(var1.pred = as.numeric(var1.pred)) %>%
  filter(!is.na(var1.pred) & is.finite(var1.pred)) %>%
  mutate(x = round(x, 5), y = round(y, 5)) %>% 
  select(x, y, var1.pred) %>% 
...
str(interp_df)
#, x = round(coords_idw_result_tern[, 1], 2), 
         #y = round(coords_idw_result_tern[, 2], 2)

ocean_cropped_F <- st_crop(ocean,xmin = 55.93, xmax = 55.955, ymin = -4.578, ymax = -4.588)
ocean_F <- st_difference(ocean_cropped_F, out_F)

map_S1 <- ggplot() +
  theme_bw() +
  coord_map() + 
  geom_contour_fill(aes(x = x, y = y, z = var1.pred, fill = after_stat(level_mid)), data = interp_df) +
  geom_sf(data = contours_clipped, aes(geometry = geometry), alpha = 0.15) +
  geom_sf(data = ocean_F, fill = "white", color = "NA") +
  geom_sf(data = out_F, fill = "transparent") +
...
d15Nmod_source <- brm(d15N ~ taxloc + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = source_SIA_mod)
...
summary(rhat(d15Nmod_source)) #gelman
geweke_res <- as.mcmc(d15Nmod_source) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
hypothesis(d15Nmod_source, "0 > taxlocSponge.Low")
hypothesis(d15Nmod_source, "taxlocTurf.High > taxlocTurf.Low")
```
...
d15N_source.pred.plot <- ggplot(data = d15N_source.pred) + 
  ...
  geom_jitter(data = source_SIA_mod %>% filter(!is.na(Source))  %>% 
                mutate(Source = case_when(Source == "Turf" ~ "2",
                                          Source == "Sponge" ~ "1",
                                          TRUE ~ NA), 
                         Source = as.numeric(Source)), aes(x = d15N, y = Source + 0.2, fill = Location),
              size = 1.1, pch = 21, col = "black", height = 0.2, alpha = 0.7) +
  ...
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.3)) +
...
d15Nmod_Turf <- brm(d15N ~ Location + Location:Depth + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = source_SIA %>% filter(Source == "Turf"))
d15Nmod_Sponge <- brm(d15N ~ Location + Location:Depth + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = source_SIA %>% filter(Source == "Sponge"))
...
summary(rhat(d15Nmod_Turf)) #gelman
geweke_res <- as.mcmc(d15Nmod_Turf) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke

summary(rhat(d15Nmod_Sponge)) #gelman
geweke_res <- as.mcmc(d15Nmod_Sponge) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke

parnames(d15Nmod_Turf)
hypothesis(d15Nmod_Turf, "LocationHigh:Depth < LocationLow:Depth")
hypothesis(d15Nmod_Turf, "LocationLow:Depth + LocationLow < 0")
```
...
crypto_SIA_mod <- data %>%
  ...
  mutate(ordervar = case_when(GENSPE == "Eviogutt" ~ 1,
                              GENSPE == "Enneabel" ~ 2,
                              TRUE ~ 3),
         taxloc = factor(interaction(GENSPE, Location)))

d15Nmod_crypto <- brm(d15N ~ 0 + taxloc + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = crypto_SIA_mod)
d15Nmod_TL <- brm(d15N ~ 0 + taxloc + TL + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = crypto_SIA_mod) #double check no influence of size
...
summary(rhat(d15Nmod_crypto)) #gelman
geweke_res <- as.mcmc(d15Nmod_crypto) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
hypothesis(d15Nmod_crypto, "taxlocEviogutt.Low < taxlocEviogutt.High")
hypothesis(d15Nmod_crypto, "taxlocEnneabel.Low < taxlocEnneabel.High")
hypothesis(d15Nmod_crypto, "taxlocCirrfila.Low < taxlocCirrfila.High")

hypothesis(d15Nmod_TL, "taxlocEnneabel.High > taxlocEnneabel.Low")
hypothesis(d15Nmod_TL, "taxlocEviogutt.High > taxlocEviogutt.Low")
hypothesis(d15Nmod_TL, "taxlocCirrfila.High > taxlocCirrfila.Low")
```
...
d15N_crypto.pred.plot <- ggplot(data = d15N_crypto.pred %>% 
  ...
  geom_jitter(data = crypto_SIA_mod %>% 
                mutate(GENSPE = case_when(GENSPE == "Eviogutt" ~ "3",
                                          GENSPE == "Enneabel" ~ "2",
                                          GENSPE == "Cirrfila" ~ "1",
                                          TRUE ~ NA), 
                         GENSPE = as.numeric(GENSPE)), aes(x = d15N, y = GENSPE + 0.2, fill = Location),
              size = 1.2, pch = 21, col = "black", height = 0.2, alpha = 0.7) +
  stat_pointinterval(aes(y = reorder(GENSPE, ordervar, decreasing = T), x = .epred, fill = Location, col = Location), 
                     point_size = 1.5, pch = 21, col = "black", position = position_dodge(width = 0.4, preserve = "single")) +
  annotation_custom(img_Eviogutt, xmin = 5, xmax = 7.1, ymin = 3, ymax = 3.5) +
  annotation_custom(img_Enneabel, xmin = 5, xmax = 7.1, ymin = 2, ymax = 2.5) +
...
d15Nmod_Enneabel <- brm(d15N ~ Location + Location:Depth + (1|Site), iter = 10000, warmup = 5000, cores = 4, 
                        data = crypto_SIA_mod %>% filter(GENSPE == "Enneabel"),
                        control = list(adapt_delta = 0.9))
d15Nmod_Eviogutt <- brm(d15N ~ Location + Location:Depth + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = crypto_SIA_mod %>% filter(GENSPE == "Eviogutt"),
                        control = list(adapt_delta = 0.9))
...
summary(rhat(d15Nmod_Enneabel)) #gelman
geweke_res <- as.mcmc(d15Nmod_Enneabel) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke

summary(rhat(d15Nmod_Eviogutt)) #gelman
geweke_res <- as.mcmc(d15Nmod_Eviogutt) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
```
...
d13Cmod_source <- brm(d13C ~ taxloc + (1|Site), data = source_SIA_mod, iter = 10000, warmup = 5000, cores = 4, 
...
summary(rhat(d13Cmod_source)) #gelman
geweke_res <- as.mcmc(d13Cmod_source) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
# Fig 2. d13C across both sources
...
d13C_source.pred.plot <- ggplot(data = d13C_source.pred) +
  stat_slab(aes(y = reorder(Source, ordervar, decreasing = T), x = .epred, fill = Location, col = Location, 
                   group = Location), height = 0.8, slab_linewidth = 0.7, orientation = "horizontal") +
  geom_jitter(data = source_SIA_mod  %>% 
                mutate(Source = case_when(Source == "Turf" ~ "2",
                                          Source == "Sponge" ~ "1",
                                          TRUE ~ NA),
                       Source = as.numeric(Source)), aes(x = d13C, y = Source + 0.2, fill = Location),
              size = 1.1, pch = 21, col = "black", height = 0.1, alpha = 0.7) +
  stat_pointinterval(aes(y = reorder(Source, ordervar, decreasing = T), x = .epred, fill = Location, col = Location), 
                     point_size = 1.5, pch = 21, col = "black", position = position_dodge(width = 0.4, preserve = "single")) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.3)) +
...
d13Cmod_Turf <- brm(d13C ~ Location + Location:Depth + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = source_SIA %>% filter(Source == "Turf"))
d13Cmod_Sponge <- brm(d13C ~ Location + Location:Depth + (1|Site), data = source_SIA %>% filter(Source == "Sponge"),
                      iter = 10000, warmup = 5000, cores = 4, control = list(adapt_delta = 0.99))

# model summary
summary(d13Cmod_Turf)
summary(d13Cmod_Sponge)

summary(rhat(d13Cmod_Turf)) #gelman
geweke_res <- as.mcmc(d13Cmod_Turf) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke

summary(rhat(d13Cmod_Sponge)) #gelman
geweke_res <- as.mcmc(d13Cmod_Sponge) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
# Fig S4. a - turf d13C according to depth
...
# Fig S4. b - sponge d13C according to depth
...
d13Cmod_crypto <- brm(d13C ~ 0 + taxloc + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = crypto_SIA_mod)
d13Cmod_TL <- brm(d13C ~ 0 + taxloc + TL + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = crypto_SIA_mod)
...
summary(rhat(d13Cmod_crypto)) #gelman
geweke_res <- as.mcmc(d13Cmod_crypto) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
hypothesis(d13Cmod_crypto, hypothesis = "taxlocEnneabel.Low < taxlocEnneabel.High")
hypothesis(d13Cmod_crypto, hypothesis = "taxlocEviogutt.Low < taxlocEviogutt.High")
hypothesis(d13Cmod_crypto, hypothesis = "taxlocCirrfila.Low < taxlocCirrfila.High")

hypothesis(d13Cmod_TL, hypothesis = "taxlocEnneabel.Low < taxlocEnneabel.High")
hypothesis(d13Cmod_TL, hypothesis = "taxlocEviogutt.Low < taxlocEviogutt.High")
hypothesis(d13Cmod_TL, hypothesis = "taxlocCirrfila.Low < taxlocCirrfila.High")
```

# Fig 2. d - d13C across all three species

```{r d13C-crypto-plot}

d13C_crypto.pred.plot <- ggplot(data = d13C_crypto.pred %>% 
                mutate(GENSPE = case_when(GENSPE == "Eviogutt" ~ "Eviota guttata",
                                          GENSPE == "Enneabel" ~ "Enneapterygius abeli",
                                          GENSPE == "Cirrfila" ~ "Cirripectes filamentosus",
                                          TRUE ~ NA_character_))) +
  stat_slab(aes(y = reorder(GENSPE, ordervar, decreasing = T), x = .epred, fill = Location, col = Location,
                   group = Location), height = 0.8, slab_linewidth = 0.7, orientation = "horizontal") +
  geom_jitter(data = crypto_SIA_mod %>% 
                mutate(GENSPE = case_when(GENSPE == "Eviogutt" ~ "3",
                                          ...
                                          GENSPE == "Cirrfila" ~ "1",
                                          TRUE ~ NA),
                       GENSPE = as.numeric(GENSPE)), aes(x = d13C, y = GENSPE + 0.2, fill = Location),
              size = 1.2, pch = 21, col = "black", height = 0.2, alpha = 0.7) +
  stat_pointinterval(aes(y = reorder(GENSPE, ordervar, decreasing = T), x = .epred, fill = Location, col = Location),
                     point_size = 1.5, pch = 21, col = "black", position = position_dodge(width = 0.4, preserve = "single")) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
...
# Fig 2 - Combine d15N and d13C across all sources and three species

```{r d13C-d15N-crypto-plot}

legend <- get_legend(d15N_source.pred.plot + theme(legend.position = "top"))

plots_combined <- ggarrange(ggarrange(
  d15N_source.pred.plot + rremove("xlab"), 
  d15N_crypto.pred.plot + theme(legend.position = "none"), 
  labels = c("a", "c"),  align = "v",
  ncol = 1, nrow = 2, legend = "none", heights = c(0.6, 1), hjust = -11),
  ggarrange(
  d13C_source.pred.plot + rremove("xlab"), 
  d13C_crypto.pred.plot + rremove("y.text"), 
  labels = c("b", "d"),  align = "v",
  ncol = 1, nrow = 2, legend = "none", heights = c(0.6, 1), hjust = 1),
  align = "hv",
  ncol = 2, nrow = 1, widths = c(1, 0.7))

crypto.pred <- plot_grid(legend, plots_combined, ncol = 1, rel_heights = c(0.1, 1))

crypto.pred

#ggsave("figures/Fig2.png", crypto.pred, width = 6, height = 6, units = "in", dpi = 600)
#ggsave("figures/Fig2.pdf", crypto.pred, width = 6, height = 6, units = "in", dpi = 600)
```
...
# model for Enneapterygius abeli
d13Cmod_Enneabel <- brm(d13C ~ Location + Location:Depth + (1|Site), iter = 10000, warmup = 5000, cores = 4, 
                        data = crypto_SIA_mod %>% filter(GENSPE == "Enneabel"))
d13Cmod_Eviogutt <- brm(d13C ~ Location + Location:Depth + (1|Site), iter = 10000, warmup = 5000, cores = 4, data = crypto_SIA_mod %>% filter(GENSPE == "Eviogutt"))
...
summary(rhat(d13Cmod_Eviogutt)) #gelman
geweke_res <- as.mcmc(d13Cmod_Eviogutt) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke

summary(rhat(d13Cmod_Enneabel)) #gelman
geweke_res <- as.mcmc(d13Cmod_Enneabel) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
```

# Fig S5. a - Enneapterygius abeli d13C according to depth
...
# Fig S5. b - Eviota guttata d13C according to depth
...
#ggsave("figures/FigS6.png", d13C.crypto.depth.pred, width = 6, height = 4, units = "in", dpi = 600)
#ggsave("figures/FigS6.pdf", d13C.crypto.depth.pred, width = 6, height = 4, units = "in", dpi = 600)
...
crypto_sources_SIA_mix_cont <- source_SIA %>% 
  select(Source, d13C, Depth, Location) %>% 
  group_by(Source, Location) %>% 
  summarize(Meand13C = mean(d13C, na.rm = T), SDd13C = sd(d13C, na.rm = T), n = n()) %>% 
  filter(Source %in% c("Turf", "Sponge")) %>% 
  mutate(Source = ifelse(Source == "Turf", "Benthic", "Pelagic")) %>% 
  ungroup()

write.csv(crypto_SIA_mix_Enneabel_cont, "output_data/crypto_SIA_mix_Enneabel_cont_one.csv", row.names = FALSE)
write.csv(crypto_SIA_mix_Eviogutt_cont, "output_data/crypto_SIA_mix_Eviogutt_cont_one.csv", row.names = FALSE)
write.csv(crypto_sources_SIA_mix_cont, "output_data/crypto_sources_SIA_mix_cont_one.csv", row.names = FALSE)

# load the mixture/consumer data
Enneabel_mix <- load_mix_data("output_data/crypto_SIA_mix_Enneabel_cont_one.csv", 
                            iso_names = c("d13C"), 
                            factors = c("Location"), 
                            fac_random = c(FALSE), 
                            fac_nested = c(FALSE), 
...
Eviogutt_mix <- load_mix_data("output_data/crypto_SIA_mix_Eviogutt_cont_one.csv", 
                            iso_names = c("d13C"), 
                            factors = c("Location"), 
                            fac_random = c(FALSE), 
                            fac_nested = c(FALSE), 
...
sources_Enneabel_mix <- load_source_data("output_data/crypto_sources_SIA_mix_cont_one.csv",
                                         source_factors = "Location", 
                                         conc_dep = FALSE, 
                                         data_type = "means", 
                                         Enneabel_mix)

sources_Eviogutt_mix <- load_source_data("output_data/crypto_sources_SIA_mix_cont_one.csv",
                                         source_factors = "Location", 
                                         conc_dep = FALSE, 
                                         data_type = "means", 
                                         Eviogutt_mix)

discr_Enneabel_mix <- load_discr_data("data/crypto_discr_SIA_mix_one.csv", Enneabel_mix)
discr_Eviogutt_mix <- load_discr_data("data/crypto_discr_SIA_mix_one.csv", Eviogutt_mix)
```
...
model_filename <- "output_data/MixSIAR_model_Enneabel_cont_one.txt"   # Name of the JAGS model file
resid_err <- TRUE
process_err <- TRUE
write_JAGS_model(model_filename, resid_err, process_err, Enneabel_mix, sources_Enneabel_mix)

# run JAGS model (~20 min)
jags.Enneabel.cont.one <- run_model(run = "extreme", 
                    Enneabel_mix, 
...
# assess model fit
output_JAGS(jags.Enneabel.cont.one, Enneabel_mix, sources_Enneabel_mix, output_options)
```

# Fig 3

```{r MM-Enneabel-plot}
g.post.Enneabel <- output_posteriors(jags.Enneabel.cont.one, Enneabel_mix, sources_Enneabel_mix, output_options)

stats <- as.data.frame(output_stats(jags.Enneabel.cont.one, Enneabel_mix, sources_Enneabel_mix, output_options)) %>% 
  select(Mean) %>% 
...
predicted_data_a <- ggplot_build(g.post.Enneabel$cont[[1]][[1]])$data[[1]]
head(predicted_data_a)

predicted_data_b <- ggplot_build(g.post.Enneabel$cont[[2]][[1]])$data[[1]]
head(predicted_data_b)
...
#ggsave("figures/Fig3.png", plot.Enneabel.mix, width = 6, height = 5, units = "in", dpi = 600)
#ggsave("figures/Fig3.pdf", plot.Enneabel.mix, width = 6, height = 5, units = "in", dpi = 600)

plot.Enneabel.mix
get_data(plot.Enneabel.mix.a)
get_data(plot.Enneabel.mix.b)

# LOO and error

observed <- crypto_SIA_mix_Enneabel_cont$d13C

posterior_samples <- jags.Enneabel.cont.one$BUGSoutput$sims.list

source_proportions <- posterior_samples$p.ind  # Source proportions from posterior
isotope_means <- crypto_sources_SIA_mix_cont$Meand13C          # Source isotopic means
isotope_sds <- crypto_sources_SIA_mix_cont$SDd13C             # Source isotopic SDs

predict_isotope <- function(proportions, isotope_means) {
  rowSums(proportions * isotope_means)  # Weighted mean of source contributions
}

predicted <- apply(source_proportions, 1, function(proportions) {
  predict_isotope(proportions, isotope_means)
})

predicted_median <- apply(predicted, 1, median)

multiplicative_errors <- observed / predicted_median

error_summary <- summary(multiplicative_errors)
print(error_summary)

hist(multiplicative_errors, main = "Distribution of Multiplicative Error Terms",
     xlab = expression(epsilon[j]), breaks = 20)

# LOO analysis

log_lik <- posterior_samples$loglik 
...
```{r MM-Enneabel-supp-plot}

plot.Enneabel.low.3 <- g.post.Enneabel$cont[[2]][[2]] +
  geom_density(lwd = 0.8, alpha = 0.3, aes(y=..scaled..)) +
  scale_color_manual(values = c("#A4C689", "#4D8BC9")) +
  scale_fill_manual(values = c("#A4C689", "#4D8BC9")) +
  labs(title = NULL) +
  ...
  theme(panel.background = element_rect(fill = "transparent"),
        axis.line = element_line(colour = "black"),
        axis.title = element_text(size = 10)) +
  ggtitle("Seabird-poor; 3m")

plot.Enneabel.low.4 <- g.post.Enneabel$cont[[2]][[3]] +
  geom_density(lwd = 0.8, alpha = 0.3, aes(y=..scaled..)) +
  scale_color_manual(values = c("#A4C689", "#4D8BC9")) +
  scale_fill_manual(values = c("#A4C689", "#4D8BC9")) +
  labs(title = NULL) +
  ...
  theme(panel.background = element_rect(fill = "transparent"),
        axis.line = element_line(colour = "black"),
        axis.title = element_text(size = 10)) +
  ggtitle("Seabird-poor; 4m")

plot.Enneabel.low.9 <- g.post.Enneabel$cont[[2]][[4]] +
  geom_density(lwd = 0.8, alpha = 0.3, aes(y=..scaled..)) +
  scale_color_manual(values = c("#A4C689", "#4D8BC9")) +
  scale_fill_manual(values = c("#A4C689", "#4D8BC9")) +
  labs(title = NULL) +
  ...
  theme(panel.background = element_rect(fill = "transparent"),
        axis.line = element_line(colour = "black"),
        axis.title = element_text(size = 10)) +
  ggtitle("Seabird-poor; 9m")

plot.Enneabel.high.3 <- g.post.Enneabel$cont[[1]][[2]] +
  geom_density(lwd = 0.8, alpha = 0.3, aes(y=..scaled..)) +
  scale_color_manual(values = c("#A4C689", "#4D8BC9")) +
  scale_fill_manual(values = c("#A4C689", "#4D8BC9")) +
  labs(title = NULL) +
  ...
  theme(panel.background = element_rect(fill = "transparent"),
        axis.line = element_line(colour = "black"),
        axis.title = element_text(size = 10)) +
  ggtitle("Seabird-rich; 3m")

plot.Enneabel.high.4 <- g.post.Enneabel$cont[[1]][[3]] +
  geom_density(lwd = 0.8, alpha = 0.3, aes(y=..scaled..)) +
  scale_color_manual(values = c("#A4C689", "#4D8BC9")) +
  scale_fill_manual(values = c("#A4C689", "#4D8BC9")) +
  labs(title = NULL) +
  ...
  theme(panel.background = element_rect(fill = "transparent"),
        axis.line = element_line(colour = "black"),
        axis.title = element_text(size = 10)) +
  ggtitle("Seabird-rich; 4m")

plot.Enneabel.high.9 <- g.post.Enneabel$cont[[1]][[4]] +
  geom_density(lwd = 0.8, alpha = 0.3, aes(y=..scaled..)) +
  scale_color_manual(values = c("#A4C689", "#4D8BC9")) +
  scale_fill_manual(values = c("#A4C689", "#4D8BC9")) +
  labs(title = NULL) +
  ...
  theme(panel.background = element_rect(fill = "transparent"),
        axis.line = element_line(colour = "black"),
        axis.title = element_text(size = 10)) +
  ggtitle("Seabird-rich; 9m")

plot.Enneabel.supp <- ggarrange(plot.Enneabel.low.3 + rremove("legend.title"), plot.Enneabel.high.3 + rremove("ylab") + rremove("legend.title"),
                               plot.Enneabel.low.4 + rremove("legend.title"), plot.Enneabel.high.4 + rremove("ylab") + rremove("legend.title"),
                               plot.Enneabel.low.9 + rremove("legend.title"), plot.Enneabel.high.9 + rremove("ylab") + rremove("legend.title"),
                               ncol = 2, nrow = 3, common.legend = T, legend = "top") 
plot.Enneabel.supp

ggsave("figures/FigS8.png", plot.Enneabel.supp, width = 6, height = 5, units = "in", dpi = 600)
ggsave("figures/FigS8.pdf", plot.Enneabel.supp, width = 6, height = 5, units = "in", dpi = 600)
...
iso.Eviogutt
# Write the JAGS model file
model_filename <- "output_data/MixSIAR_model_Eviogutt_cont_one.txt"   
resid_err <- TRUE
process_err <- TRUE
write_JAGS_model(model_filename, resid_err, process_err, Eviogutt_mix, sources_Eviogutt_mix)

# run JAGS model (~20 min)
jags.Eviogutt.cont.one <- run_model(run = "extreme", 
                    Eviogutt_mix, 
                    sources_Eviogutt_mix, 
                    discr_Eviogutt_mix, model_filename)

# assess model fit
output_JAGS(jags.Eviogutt.cont.one, Eviogutt_mix, sources_Eviogutt_mix, output_options)
```

# Fig S7

```{r MM-Eviogutt-plot}
g.post <- output_posteriors(jags.Eviogutt.cont.one, Eviogutt_mix, sources_Eviogutt_mix, output_options)

stats <- as.data.frame(output_stats(jags.Eviogutt.cont.one, Eviogutt_mix, sources_Eviogutt_mix, output_options)) %>% 
  select(Mean) %>% 
  rownames_to_column("Source") %>% 
  separate(Source, c("val", "Location", "Source")) %>% 
  pivot_wider(names_from = Location, values_from = Mean) %>% 
...
ggsave("figures/FigS7.png", plot.Eviogutt.mix, width = 6, height = 5, units = "in", dpi = 600)
ggsave("figures/FigS7.pdf", plot.Eviogutt.mix, width = 6, height = 5, units = "in", dpi = 600)

get_data(plot.Eviogutt.mix.a)
get_data(plot.Eviogutt.mix.b)

plot.Eviogutt.mix

observed <- crypto_SIA_mix_Eviogutt_cont$d13C

posterior_samples <- jags.Eviogutt.cont.one$BUGSoutput$sims.list
...
Spr.mod <- brm(Spr_m ~ Location + Complexity + Location:Depth, family = lognormal, iter = 10000, warmup = 5000, cores = 4,data = data_site)
Dens.mod <- brm(Ab_m ~ Location + Complexity + Location:Depth, family = lognormal, iter = 10000, warmup = 5000, cores = 4, data = data_site)
Biomass_m.mod <- brm(Biomass_m ~ Location + Complexity + Location:Depth, family = lognormal, iter = 10000, warmup = 5000, cores = 4, data = data_site)

# assess fit
pp_check(Spr.mod)
pp_check(Dens.mod)
pp_check(Biomass_m.mod)

summary(rhat(Spr.mod)) #gelman
geweke_res <- as.mcmc(Spr.mod) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke

summary(rhat(Dens.mod)) #gelman
geweke_res <- as.mcmc(Dens.mod) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke

summary(rhat(Biomass_m.mod)) #gelman
geweke_res <- as.mcmc(Biomass_m.mod) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke

# predict values for both levels of Location
...
Spr.pred.plot <- ggplot(data = Spr.pred %>% filter(.epred < 10)) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location),
            height = 0.8, slab_linewidth = 0.7, orientation = "horizontal") +
  geom_jitter(data = data_site, aes(y = 0.2, x = Spr_m, fill = Location),
              size = 1.2, pch = 21, col = "black", height = 0.1, alpha = 0.7) +
...
Dens.pred.plot <- ggplot(data = Dens.pred %>% filter(.epred < 30)) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location), height = 0.8, slab_linewidth = 0.7, orientation = "horizontal") +
  geom_jitter(data = data_site, aes(y = 0.2, x = Ab_m, fill = Location),
              size = 1.2, pch = 21, col = "black", height = 0.1, alpha = 0.7) +
  stat_pointinterval(aes(x = .epred, fill = Location, col = Location), 
                     point_size = 1.5, pch = 21, col = "black", position = position_dodge(width = 0.4, preserve = "single")) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
...
Biomass_m.pred.plot <- ggplot(data = Biomass_m.pred %>% filter(.epred < 9)) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location),  height = 0.8, slab_linewidth = 0.7, orientation = "horizontal") +
  geom_jitter(data = data_site, aes(y = 0.2, x = Biomass_m, fill = Location),
              size = 1.2, pch = 21, col = "black", height = 0.1, alpha = 0.7) +
  stat_pointinterval(aes(x = .epred, fill = Location, col = Location), 
...
#ggsave("figures/FigS9.png", com_estimates.plot, width = 6, height = 5, units = "in", dpi = 600)
#ggsave("figures/FigS9.pdf", com_estimates.plot, width = 6, height = 5, units = "in", dpi = 600)
```
...
# Eviobipu
4.77/sum(species_scores[,1])
#Enneabel
4.24/sum(species_scores[,1])

#ggsave("figures/FigS10.png", rda_plot, width = 6, height = 6, units = "in", dpi = 600)
#ggsave("figures/FigS10.pdf", rda_plot, width = 6, height = 6, units = "in", dpi = 600)
```
...
TL_mod_com <- brm(TL ~ Location + (1|GENSPE) + (1|Site), family = lognormal, iter = 10000, warmup = 5000, cores = 4, data = TL_mod_com.data)
...
summary(rhat(TL_mod_com)) #gelman
geweke_res <- as.mcmc(TL_mod_com) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
TL_mod <- brm(TL ~ 0 + taxloc + (1|Site), family = lognormal, iter = 10000, warmup = 5000, cores = 4, data = TL_mod.data)
...
summary(rhat(TL_mod)) #gelman
geweke_res <- as.mcmc(TL_mod) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
contrasts_TL
hypothesis(TL_mod, "taxlocEviogutt.Low < taxlocEviogutt.High")
hypothesis(TL_mod, "taxlocEnneabel.Low < taxlocEnneabel.High")
hypothesis(TL_mod, "taxlocEviobipu.Low < taxlocEviobipu.High")
```

# Fig 3.a - Total length
...
TL.pred.plot <- ggplot(data = TL.pred %>% 
  ...
  geom_jitter(data = TL_mod.data %>% 
                                   mutate(GENSPE = case_when(GENSPE == "Enneabel" ~ "1",
                                          GENSPE == "Eviogutt" ~ "2",
                                          GENSPE == "Eviobipu" ~ "3",
                                          TRUE ~ NA),
                                          GENSPE = as.numeric(GENSPE)), 
              aes(x = TL, y = GENSPE + 0.2, fill = Location),
              size = 1.2, pch = 21, col = "black", height = 0.2, alpha = 0.7) +
  stat_pointinterval(aes(y = reorder(GENSPE, ordervar, decreasing = T), x = .epred, fill = Location, col = Location), 
                     point_size = 1.5, pch = 21, col = "black", position = position_dodge(width = 0.4, preserve = "single")) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
...
# model
W_mod_com <- brm(W ~ Location + (1|GENSPE) + (1|Site), family = lognormal, iter = 10000, warmup = 5000, cores = 4, data = data_red)
...
summary(rhat(W_mod_com)) #gelman
geweke_res <- as.mcmc(W_mod_com) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
W_mod <- brm(W ~ 0 + taxloc + (1|Site), iter = 10000, warmup = 5000, family = lognormal, data = W_mod.data)
...
summary(rhat(W_mod)) #gelman
geweke_res <- as.mcmc(W_mod) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
contrasts_W
hypothesis(W_mod, "taxlocEviogutt.Low < taxlocEviogutt.High")
hypothesis(W_mod, "taxlocEnneabel.Low < taxlocEnneabel.High")
hypothesis(W_mod, "taxlocEviobipu.Low < taxlocEviobipu.High")
```
...
W.pred.plot <- ggplot(data = W.pred %>% 
                                   mutate(GENSPE = case_when(GENSPE == "Enneabel" ~ "Enneapterygius abeli",
                                          ...
                                          TRUE ~ NA_character_))) +
  stat_slab(aes(y = reorder(GENSPE, ordervar, decreasing = T), x = .epred, fill = Location, col = Location,
                   group = Location), height = 1, slab_linewidth = 0.7, orientation = "horizontal") +
  geom_jitter(data = W_mod.data %>% 
                               mutate(GENSPE = case_when(GENSPE == "Enneabel" ~ "1",
                                      ...
                                      TRUE ~ NA),
                                      GENSPE = as.numeric(GENSPE)), 
              aes(x = W, y = GENSPE + 0.2, fill = Location),
              size = 1.2, pch = 21, col = "black", height = 0.2, alpha = 0.7) +
  stat_pointinterval(aes(y = reorder(GENSPE, ordervar, decreasing = T), x = .epred, fill = Location, col = Location), 
                     point_size = 1.5, pch = 21, col = "black", position = position_dodge(width = 0.4, preserve = "single")) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
...
# Fig 3 - Combine length and weight
...
#ggsave("figures/Fig4.png", TL_W.plot, width = 6, height = 4, units = "in", dpi = 600)
#ggsave("figures/Fig4.pdf", TL_W.plot, width = 6, height = 4, units = "in", dpi = 600)
...
ggsave("figures/FigS11.png", rda_uvc.plot, width = 6, height = 6, units = "in", dpi = 600)
ggsave("figures/FigS11.pdf", rda_uvc.plot, width = 6, height = 6, units = "in", dpi = 600)
```
...
priors <- c(
  prior(normal(0, 1), class = "b"),                # Predictors with small/moderate effects
  prior(normal(log(100), 1), class = "Intercept"), # Narrow intercept prior
  prior(normal(1, 0.5), class = "sigma", lb = 0, ub = 1)   # Narrow residual SD
)

#prior(normal(4.4, 0.3), class = "Intercept"), # Centered on log-median of data
#            # Tight prior for coefficients


# get_prior(uvc_biomass_all.mod)
uvc_biomass_all.prior <- brm(Biomass_m ~ Location, family = lognormal, prior = priors, iter = 10000, warmup = 5000, cores = 4, data = uvc_data_biomass, sample_prior = "only")
# 
pred_prior <- posterior_predict(uvc_biomass_all.prior, ndraws = 100)
summary(pred_prior)
#summary(uvc_biomass_all.prior)
pp_check(uvc_biomass_all.prior, ndraws = 10)
  

# model
uvc_biomass_all.mod <- brm(Biomass_m ~ Location, family = lognormal, prior = priors, iter = 10000, warmup = 5000, cores = 4, data = uvc_data_biomass)
...
summary(rhat(uvc_biomass_all.mod)) #gelman
geweke_res <- as.mcmc(uvc_biomass_all.mod) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
uvc_biomass.pred.plot.all <- ggplot(data = uvc_biomass_all.pred) +
  ...
  geom_jitter(data = uvc_data_biomass, 
              aes(x = Biomass_m, y = 0.2, fill = Location),
              size = 1.5, pch = 21, col = "black", height = 0.2, alpha = 0.7) +
  stat_pointinterval(aes(x = .epred, fill = Location, col = Location), 
                     point_size = 1.5, pch = 21, col = "black", position = position_dodge(width = 0.4, preserve = "single")) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.3)) +
  ...
  theme(panel.background = element_rect(fill = "transparent"),
        axis.line = element_line(colour = "black"),
        axis.title.y = element_blank(),
        plot.title = element_text(size = 11, hjust = 1, vjust = 0.7, margin = margin(t = 10, b = -10))) +
...
uvc_biomass.mod <- brm(Biomass_m ~ taxloc, family = lognormal, prior = priors, iter = 10000, warmup = 5000, cores = 4, data = uvc_biomass.mod_data)
...
summary(rhat(uvc_biomass.mod)) #gelman
geweke_res <- as.mcmc(uvc_biomass.mod) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
# Fig 5 - Biomass across all functional groups

```{r uvc-biomass-FG-plot}

uvc_biomass.pred.plot.Herb <- ggplot(data = uvc_biomass.pred %>% filter(FG == "Herbivore")) +
  annotation_custom(img_Herb, xmin = 520, xmax = 620, ymin = 0.3, ymax = 0.8) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location), orientation = "horizontal") +
  ...
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.3)) +
  annotate("text", x = 550, y = 1, label = "Herbivore") +
  labs(x = bquote("Biomass (g /" ~m^2~")")) +
  guides(fill = guide_legend(title = "Seabird-nutrient load"),
         col = guide_legend(title = "Seabird-nutrient load")) +
  theme(panel.background = element_rect(fill = "transparent"),
        axis.line = element_line(colour = "black"),
        axis.title.y = element_blank()) +
  coord_cartesian(xlim = c(0, 600), clip = "off")

uvc_biomass.pred.plot.Omni <- ggplot(data = uvc_biomass.pred %>% filter(FG == "Omnivore")) +
  annotation_custom(img_Omni, xmin = 17, xmax = 21, ymin = 0.3, ymax = 0.8) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location), orientation = "horizontal") +
...
uvc_biomass.pred.plot.Inv <- ggplot(data = uvc_biomass.pred %>% filter(FG == "Invertivore")) +
  annotation_custom(img_Inv, xmin = 30, xmax = 37, ymin = 0.3, ymax = 0.8) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location), orientation = "horizontal") +
  ...
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.3)) +
  annotate("text", x = 32, y = 1, label = "Invertivore") +
  labs(x = bquote("Biomass (g /" ~m^2~")")) +
  guides(fill = guide_legend(title = "Seabird-nutrient load"),
         col = guide_legend(title = "Seabird-nutrient load")) +
  theme(panel.background = element_rect(fill = "transparent"),
...
uvc_biomass.pred.plot.Carn <- ggplot(data = uvc_biomass.pred %>% filter(FG == "Carnivore")) +
  annotation_custom(img_Carn, xmin = 195, xmax = 260, ymin = 0.3, ymax = 0.8) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location), orientation = "horizontal") +
  geom_jitter(data = uvc_biomass.mod_data %>% filter(FG == "Carnivore"), 
              aes(x = Biomass_m, y = 0.2, fill = Location),
              size = 1.5, pch = 21, col = "black", height = 0.3, alpha = 0.8) +
  stat_pointinterval(aes(x = .epred, fill = Location, col = Location), 
                     point_size = 1.5, pch = 21, col = "black", position = position_dodge(width = 0.4, preserve = "single")) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.3)) +
  annotate("text", x = 220, y = 1, label = "Carnivore") +
  labs(x = bquote("Biomass (g /" ~m^2~")")) +
  guides(fill = guide_legend(title = "Seabird-nutrient load"),
         col = guide_legend(title = "Seabird-nutrient load")) +
  theme(panel.background = element_rect(fill = "transparent"),
...
uvc_biomass.pred.plot.Pisc <- ggplot(data = uvc_biomass.pred %>% filter(FG == "Piscivore")) +
  annotation_custom(img_Pisc, xmin = 70, xmax = 85, ymin = 0.3, ymax = 0.8) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location), orientation = "horizontal") +
  ...
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.3)) +
  annotate("text", x = 75, y = 1, label = "Piscivore") +
  labs(x = bquote("Biomass (g /" ~m^2~")")) +
  guides(fill = guide_legend(title = "Seabird-nutrient load"),
         col = guide_legend(title = "Seabird-nutrient load")) +
  theme(panel.background = element_rect(fill = "transparent"),
...
uvc_biomass.pred.plot.Cora <- ggplot(data = uvc_biomass.pred %>% filter(FG == "Corallivore")) +
  annotation_custom(img_Cora, xmin = 13.5, xmax = 17, ymin = 0.25, ymax = 0.8) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location), orientation = "horizontal") +
  ...
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.5)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.3)) +
  annotate("text", x = 14.9, y = 1, label = "Corallivore") +
  labs(x = bquote("Biomass (g /" ~m^2~")")) +
  guides(fill = guide_legend(title = "Seabird-nutrient load"),
         col = guide_legend(title = "Seabird-nutrient load")) +
  theme(panel.background = element_rect(fill = "transparent"),
...
uvc_biomass.pred.plot.Plan <- ggplot(data = uvc_biomass.pred %>% filter(FG == "Planktivore")) +
  annotation_custom(img_Plan, xmin = 75, xmax = 120, ymin = 0.25, ymax = 0.8) +
  stat_slab(aes(x = .epred, fill = Location, col = Location,
                   group = Location), orientation = "horizontal") +
  ...
  annotate("text", x = 90, y = 1, label = "Planktivore") +
  labs(x = bquote("Biomass (g /" ~m^2~")")) +
  guides(fill = guide_legend(title = "Seabird-nutrient load"),
         col = guide_legend(title = "Seabird-nutrient load")) +
  theme(panel.background = element_rect(fill = "transparent"),
...
uvc_biomass.mod_depth <- brm(Biomass_m ~ Location + Location:Depth, family = lognormal, prior = priors, iter = 10000, warmup = 5000, cores = 4, 
                             data = uvc_data_biomass)

summary(uvc_biomass.mod_depth)
pp_check(uvc_biomass.mod_depth)

summary(rhat(uvc_biomass.mod_depth)) #gelman
geweke_res <- as.mcmc(uvc_biomass.mod_depth) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
uvc_biomass.mod_depth.pred <- uvc_data_biomass %>% 
  group_by(Location) %>%  
  data_grid(Depth = seq_range(0:12, n = 100)) %>%
  add_epred_draws(uvc_biomass.mod_depth, ndraws = 1000, re_formula = NA, allow.new.levels = T)

uvc_biomass.mod_depth.pred.plot1 <- ggplot(uvc_biomass.mod_depth.pred, aes(x = Depth, y = Biomass_m, color = Location)) +
  stat_lineribbon(aes(y = .epred, color = Location, fill = Location), .width = .50) +
  geom_point(data = uvc_data_biomass, color = "black", size = 2.2) +
  geom_point(data = uvc_data_biomass, aes(color = Location), size = 2) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.7)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.2)) +
  annotate("text", x = 10.5, y = 620, label = "All") +
  theme(axis.line = element_line(colour = "black"),
  ...
  labs(y = bquote("Biomass (g/" ~m^2~")"),
       x = "Depth (m)") +
  guides(col = "none",
         fill = guide_legend(title = "50% CI")) +
  coord_cartesian(ylim = c(0,650), xlim = c(0, 11))

uvc_biomass.mod_depth.pred.plot
uvc_biomass.mod_depth.pred.plot1
hypothesis(uvc_biomass.mod_depth, "LocationHigh:Depth < LocationLow:Depth")
hypothesis(uvc_biomass.mod_depth, "LocationHigh:Depth < 0")
hypothesis(uvc_biomass.mod_depth, "LocationLow:Depth < 0")
```

```{r uvc-biomass-depth-herb}

uvc_biomass.mod_Herb <- brm(Biomass_m ~ Location + Location:Depth, family = lognormal, prior = priors, iter = 10000, warmup = 5000, cores = 4, data = uvc_data_biomass_FG %>% 
                              filter(FG == "Herbivore"))

summary(uvc_biomass.mod_Herb)
pp_check(uvc_biomass.mod_Herb)

summary(rhat(uvc_biomass.mod_Herb)) #gelman
geweke_res <- as.mcmc(uvc_biomass.mod_Herb) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke
...
uvc_biomass.mod_Herb.pred.plot <- ggplot(uvc_biomass.mod_Herb.depth.pred, aes(x = Depth, y = Biomass_m, color = Location)) +
  stat_lineribbon(aes(y = .epred, color = Location, fill = Location), .width = .50) +
  geom_point(data = uvc_data_biomass_FG %>% filter(FG == "Herbivore"), color = "black", size = 2.2) +
  geom_point(data = uvc_data_biomass_FG %>% filter(FG == "Herbivore"), aes(color = Location), size = 2) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.7)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.2)) +
  annotate("text", x = 9.5, y = 600, label = "Herbivore") +
...
uvc_biomass.mod_Herb.pred.plot

hypothesis(uvc_biomass.mod_Herb, "LocationHigh:Depth < LocationLow:Depth")
hypothesis(uvc_biomass.mod_Herb, "LocationHigh:Depth < 0")
hypothesis(uvc_biomass.mod_Herb, "LocationLow:Depth < 0")
```


```{r uvc-biomass-depth-other}

uvc_biomass.mod_other <- brm(Biomass_m ~ Location + Location:Depth, family = lognormal, prior = priors, iter = 10000, warmup = 5000, cores = 4, data = uvc_data_biomass_FG %>% 
                              filter(FG != "Herbivore") %>% group_by(Site) %>% mutate(Biomass_m = sum(Biomass_m, na.rm = T)) %>% 
                               distinct(Site, Depth, Biomass_m, Location))

summary(uvc_biomass.mod_other)
pp_check(uvc_biomass.mod_other)

summary(rhat(uvc_biomass.mod_other)) #gelman
geweke_res <- as.mcmc(uvc_biomass.mod_other) %>% geweke.diag()
z_scores <- c(geweke_res[[1]]$z, geweke_res[[2]]$z, geweke_res[[3]]$z, geweke_res[[4]]$z)
sum(c(z_scores < -1.96, z_scores > 1.96))/120 #geweke

# posterior prediction for 1000 draws
uvc_biomass.mod_other.depth.pred <- uvc_data_biomass_FG %>% 
                              filter(FG != "Herbivore") %>% group_by(Site) %>% mutate(Biomass_m = sum(Biomass_m, na.rm = T)) %>% 
                               distinct(Site, Depth, Biomass_m, Location) %>% ungroup() %>% 
  group_by(Location) %>% 
  data_grid(Depth = seq_range(0:12, n = 100)) %>%
  add_epred_draws(uvc_biomass.mod_other, ndraws = 1000, re_formula = NA, allow.new.levels = T)

uvc_biomass.mod_other.pred.plot <- ggplot(uvc_biomass.mod_other.depth.pred, aes(x = Depth, y = Biomass_m, color = Location)) +
  stat_lineribbon(aes(y = .epred, color = Location, fill = Location), .width = .50) +
  geom_point(data = uvc_data_biomass_FG %>% 
                              filter(FG != "Herbivore") %>% group_by(Site) %>% mutate(Biomass_m = sum(Biomass_m, na.rm = T)) %>% 
                               distinct(Site, Depth, Biomass_m, Location), color = "black", size = 2.2) +
  geom_point(data = uvc_data_biomass_FG %>% 
                              filter(FG != "Herbivore") %>% group_by(Site) %>% mutate(Biomass_m = sum(Biomass_m, na.rm = T)) %>% 
                               distinct(Site, Depth, Biomass_m, Location), 
             aes(color = Location), size = 2) +
  scale_color_manual(values = alpha(c("#0067A4", "#BF0033"), 0.7)) +
  scale_fill_manual(values = alpha(c("#0067A4", "#BF0033"), 0.2)) +
  annotate("text", x = 7.5, y = 500, label = "Other feeding group") +
  theme(legend.position = c(0.87, 0.7)) +
  labs(y = bquote("Biomass (g/" ~m^2~")"),
       x = "Depth (m)") +
  guides(col = "none",
...
uvc_biomass.mod_other.pred.plot
```

```{r uvc-biomass-depth-plot}

uvc_biomass_depth.plot <- ggarrange(
  ggarrange(uvc_biomass.pred.plot.all + rremove("x.title"),
            uvc_biomass.pred.plot.Herb + rremove("x.title"),
            uvc_biomass.pred.plot.Omni + rremove("x.title"),
            uvc_biomass.pred.plot.Inv + rremove("x.title"),
            uvc_biomass.pred.plot.Carn + rremove("x.title"),
            uvc_biomass.pred.plot.Pisc + rremove("x.title"),
            uvc_biomass.pred.plot.Cora + rremove("x.title"),
            uvc_biomass.pred.plot.Plan,
            nrow = 8, ncol = 1, heights = c(1.2, rep(1, 6), 1.3), 
            common.legend = T, legend = "top"),
  ggarrange(uvc_biomass.mod_depth.pred.plot + rremove("x.title"),
            uvc_biomass.mod_Herb.pred.plot + rremove("x.title"),
            uvc_biomass.mod_other.pred.plot,
            nrow = 3, ncol = 1, common.legend = T, legend = "top"),
  nrow = 1, ncol = 2, labels = c("a", "b"), hjust = c(0, -3), vjust = c(4, 4))

uvc_biomass_depth.plot

#ggsave("figures/Fig5.png", uvc_biomass_depth.plot, width = 6, height = 9, units = "in", dpi = 600)
#ggsave("figures/Fig5.pdf", uvc_biomass_depth.plot, width = 6, height = 9, units = "in", dpi = 600)
```
