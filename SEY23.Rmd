...
source_SIA <- read.csv("data/SEY23_Source_SIA_data.csv") %>% 
  ...
  mutate(Source = str_extract(Source, "[:alpha:]+"),
         PercentN = as.numeric(str_extract(PercentN, "[:digit:]+[.][:digit:]+")),
         PercentC = as.numeric(str_extract(PercentC, "[:digit:]+[.][:digit:]+"))) %>% 
...
d13C_source.pred <- source_SIA_mod %>%
  data_grid(taxloc, Site) %>%  
  add_epred_draws(d13Cmod_source, ndraws = 1000) %>%
  separate(taxloc, c("Source", "Location"), remove = F) %>%
  mutate(ordervar = case_when(Source == "Turf" ~ 1,
                              TRUE ~ 2))
...
uvc_biomass.mod_Pisc.pred.plot
```
...
uvc_biomass.mod_Plan.pred.plot
```
...
uvc_biomass.mod_Omni.pred.plot
```
...
uvc_biomass.mod_Cora.pred.plot
```
...
uvc_biomass.mod_Carn.pred.plot
```
